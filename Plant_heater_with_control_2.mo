...
model Plant_heater_with_control_2
  ...
  AES.ProcessComponents.Thermal.Liquid.Tube tubeheat annotation(
    Placement(visible = true, transformation(origin = {-148, -50}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow Hsupz2 annotation(
    Placement(visible = true, transformation(origin = {398, 92}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor sTz2 annotation(
    Placement(visible = true, transformation(origin = {318, 108}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor Cz2(C = 1e4, T(displayUnit = "K")) annotation(
    Placement(visible = true, transformation(origin = {494, 108}, extent = {{-20, -20}, {20, 20}}, rotation = -90)));
  ...
  Modelica.Blocks.Math.Gain Psupz2(k = 500) annotation(
    Placement(visible = true, transformation(origin = {376, 70}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.RealExpression Pressure_difference(y = 2.027 * 10 ^ 5) annotation(
    Placement(visible = true, transformation(origin = {-367, 38}, extent = {{-25, -18}, {25, 18}}, rotation = 0)));
  ...
  AES.ControlBlocks.ActuationSchemes.DaisyChain_uniform daisyChain_z1 annotation(
    Placement(visible = true, transformation(origin = {30, 122}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  AES.ControlBlocks.ActuationSchemes.DaisyChain_uniform daisyChain_z2 annotation(
    Placement(visible = true, transformation(origin = {318, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.CombiTimeTable Tamb(extrapolation = Modelica.Blocks.Types.Extrapolation.Periodic, offset = {273.15}, smoothness = Modelica.Blocks.Types.Smoothness.ContinuousDerivative, table = [0, 0; 4, -2; 8, 8; 12, 10; 15, 10; 18, 3; 20, 1; 22, 0; 24, 0], timeScale = 3600) annotation(
    Placement(visible = true, transformation(origin = {-138, 158}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  AES.ControlBlocks.AnalogueControllers.PI_awfb_basic night_PI_z2(CSmax = 10, CSmin = 0, K = 0.005, Ti = 500) annotation(
  ...
  Modelica.Blocks.Sources.BooleanExpression booleanExpression(y = if AES_project_2021_2022.NotteGiorno(time) == 0 then false else true)  annotation(
    Placement(visible = true, transformation(origin = {168, 168}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  AES.ControlBlocks.AnalogueControllers.PI_awfb_basic night_PI_z1(CSmax = 1, CSmin = 0, K = 0.005, Ti = 500) annotation(
    Placement(visible = true, transformation(origin = {-2, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.CombiTimeTable hours_switch(extrapolation = Modelica.Blocks.Types.Extrapolation.Periodic, smoothness = Modelica.Blocks.Types.Smoothness.ConstantSegments, table = [0, 0; 7, 1; 22, 0; 24, 0], tableOnFile = false, timeScale = 3600)  annotation(
...
equation
  ...
  connect(tubeheat.pwh_b, pump.pwh_a) annotation(
    Line(points = {{-148, -38}, {-148, -30}, {-44, -30}}, color = {46, 52, 54}));
  connect(pressuriser.pwh_a, tubeheat.pwh_a) annotation(
    Line(points = {{-44, -92}, {-148, -92}, {-148, -62}}, color = {46, 52, 54}));
  ...
  connect(sTh.pwh_a, tubeheat.pwh_b) annotation(
    Line(points = {{-162, -30}, {-148, -30}, {-148, -38}}, color = {46, 52, 54}));
  connect(Qheat.surf, tubeheat.surf) annotation(
    Line(points = {{-218, -51}, {-162, -51}, {-162, -50}, {-153, -50}}, color = {144, 5, 5}));
  ...
  connect(convz2.HP, Cz2.port) annotation(
    Line(points = {{456, 72}, {456, 108}, {474, 108}}, color = {191, 0, 0}));
  connect(Cz2.port, Hsupz2.port) annotation(
    Line(points = {{474, 108}, {398, 108}, {398, 102}}, color = {191, 0, 0}));
  connect(sTz2.port, Cz2.port) annotation(
    Line(points = {{328, 108}, {474, 108}}, color = {191, 0, 0}));
  ...
  connect(thermalConductor.port_b, Cz2.port) annotation(
    Line(points = {{456, 122}, {456, 108}, {474, 108}}, color = {191, 0, 0}));
  connect(PI_z1.CS, daisyChain_z1.CSi01) annotation(
    Line(points = {{6, 122}, {18, 122}}, color = {0, 0, 127}));
  connect(gain.y, Qheat.Q) annotation(
    Line(points = {{-256, -40}, {-241, -40}, {-241, -42}, {-226, -42}}, color = {0, 0, 127}));
  connect(sp_Tz.y[1], PI_z1.SP) annotation(
    Line(points = {{-101, 128}, {-18, 128}}, color = {0, 0, 127}));
  connect(sp_Tz.y[1], PI_z2.SP) annotation(
    Line(points = {{-101, 128}, {-43, 128}, {-43, 148}, {250, 148}}, color = {0, 0, 127}));
  connect(Tamb.y[1], pTa.T) annotation(
    Line(points = {{-127, 158}, {-14, 158}}, color = {0, 0, 127}));
  ...
  connect(sTh.oT, PI_Heater.PV) annotation(
    Line(points = {{-172, -30}, {-394, -30}, {-394, -44}, {-324, -44}}, color = {0, 0, 127}));
  connect(daisyChain_z1.CSo01[2], Psupz1.u) annotation(
    Line(points = {{42, 122}, {74, 122}, {74, 36}, {113, 36}}, color = {0, 0, 127}));
  ...
  connect(Pressure_difference.y, PI_Pressure.SP) annotation(
    Line(points = {{-339.5, 38}, {-233.75, 38}, {-233.75, 6}, {-126, 6}}, color = {0, 0, 127}));
  connect(sTz2.T, night_PI_z2.PV) annotation(
    Line(points = {{308, 108}, {234, 108}, {234, 10}, {256, 10}}, color = {0, 0, 127}));
  ...
  connect(PI_z2.PV, sTz2.T) annotation(
    Line(points = {{250, 138}, {234, 138}, {234, 108}, {308, 108}}, color = {0, 0, 127}));
  ...
  connect(PI_z1.PV, sTz1.T) annotation(
    Line(points = {{-18, 118}, {-60, 118}, {-60, 86}, {84, 86}}, color = {0, 0, 127}));
  connect(daisyChain_z2.CSo01[2], Psupz2.u) annotation(
    Line(points = {{330, 70}, {371, 70}}, color = {0, 0, 127}));
  connect(daisyChain_z2.CSo01[1], switch2.u1) annotation(
    Line(points = {{330, 70}, {330, 30}, {338, 30}}, color = {0, 0, 127}));
  connect(PI_z2.CS, daisyChain_z2.CSi01) annotation(
    Line(points = {{274, 142}, {298, 142}, {298, 70}, {306, 70}}, color = {0, 0, 127}));
  connect(Psupz2.y, Hsupz2.Q_flow) annotation(
    Line(points = {{380, 70}, {398, 70}, {398, 82}}, color = {0, 0, 127}));
  connect(daisyChain_z1.CSo01[1], switch1.u1) annotation(
    Line(points = {{42, 122}, {46, 122}, {46, 28}, {48, 28}}, color = {0, 0, 127}));
  annotation(
